...
// PROCESS USER INPUT
//initialize object to store user-controlled variables
//keeping a local copy here so that the program doesn't have to 
//make a fetch request every time it makes a firework!
var properties = {};
...
window.onload = function() {
    if(window.location.href.includes("canvas")) { //on canvas page
        ...
        fetch("properties", {method: "GET"}).then(response => response.json()).then(json => {
            ...
            checkBox("greens", json.greens);
            checkBox("blues", json.blues);
            checkBox("purples", json.purples);
            checkBox("pinks", json.pinks);
            checkBox("browns", json.browns);
            checkBox("stardust", json.stardust);
            checkBox("comet", json.comet);
            checkBox("ring", json.ring);
            checkBox("bouquet", json.bouquet);
            checkBox("spiral", json.spiral);
            checkBox("peony", json.peony);
            checkBox("dahlia", json.dahlia);
            checkBox("crossette", json.crossette);
            checkBox("airResistance", json.airResistance);
            fillNumber("windspeed", json.windspeed);
            fillNumber("gravity", json.gravity);
            checkBox("xyFlip", json.xyFlip);
            fillNumber("launchSpeed", json.launchSpeed);
            fillNumber("launchAngle", json.launchAngle);
            fillNumber("fuse", json.fuse);
            fillNumber("radius", json.radius);
            fillNumber("length", json.length);
            fillNumber("duration", json.duration);
            fillNumber("explodeSpeed", json.explodeSpeed);
            fillNumber("ringParticles", json.ringParticles);
            fillNumber("bouquetParticles", json.bouquetParticles);
            fillNumber("spiralParticles", json.spiralParticles);
            fillNumber("crossetteBursts", json.crossetteBursts);
            fillNumber("spiralLoops", json.spiralLoops);
            fillNumber("emitterRings", json.emitterRings);
        }).then(console.log("loaded properties from server"));
    }
...
const toggle = function(propName) {
    ...
    updateServer(propName, newSetting);
}
...
//update server with changed property
const updateServer = function(propName, newSetting) {
    var updateData = {
        propName: propName,
    ...
    }).then(console.log("updated", propName, newSetting));
}
